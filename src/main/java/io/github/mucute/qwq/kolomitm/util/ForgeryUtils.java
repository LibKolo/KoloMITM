...
import io.github.mucute.qwq.kolomitm.data.auth.DualPayload;
...
public class ForgeryUtils {
    ...
    public static AuthPayload forgeOnlineAuthData(KoloMITM koloMITM, BedrockAuthManager authManager, ECPublicKey mojangPublicKey) throws InvalidJwtException, JoseException {
    ...
    public static String forgeOnlineSkinData(BedrockAuthManager bedrockAuthManager, JSONObject skinData, InetSocketAddress serverAddress) {
        String publicKeyBase64 = Base64.getEncoder().encodeToString(bedrockAuthManager.getSessionKeyPair().getPublic().getEncoded());

        HashMap<String,Object> overrideData = new HashMap<String,Object>();
        overrideData.put("DeviceId", bedrockAuthManager.getDeviceId().toString().replace("-", ""));
        overrideData.put("DeviceOS", 1); // Android per MinecraftAuth 4.0
        overrideData.put("ThirdPartyName", bedrockAuthManager.getMinecraftMultiplayerToken().getCached().getDisplayName());
        ...
        jws.setHeader(HeaderParameterNames.X509_URL, publicKeyBase64);
        jws.setPayload(skinData.toJSONString());
        jws.setKey(bedrockAuthManager.getSessionKeyPair().getPrivate());
